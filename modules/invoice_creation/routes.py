...
def get_business_partners():
    ...
    try:
        ...
        if not sap.base_url or not sap.username or not sap.password:
            logging.warning("SAP B1 configuration missing - returning fallback customers")
...
def validate_serial_number():
    ...
    try:
        ...
        # Check SAP configuration first
        if not sap.base_url or not sap.username or not sap.password:
            logging.warning("SAP B1 configuration missing - using fallback validation")
...
def delete_invoice(invoice_id):
    ...
    try:
        ...
        logging.info(f" Invoice {invoice_id_num} deleted successfully")
        return jsonify({
            'success': True, 
...
def delete_line_item(line_id):
    ...
    try:
        ...
        if serial_item:
            ...
            logging.info(f" Serial item {serial_item.serial_number} deleted from invoice {invoice.id}")
            return jsonify({
                'success': True, 
            ...
            logging.info(f" Invoice line item {line_id} deleted from invoice {invoice.id}")
            return jsonify({
                'success': True, 
    ...
    try:
        ...
        if not item_code:
            ...
            try:
                ...
                logging.warning(f" Validation during add failed, using provided data: {e}")
...
def remove_serial_item(item_id):
    ...
    try:
        ...
        if len(session['invoice_items']) < original_length:
            logging.info(f" Removed serial item with ID: {item_id}")
            return jsonify({
                'success': True,
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if results:
                        ...
                        logging.warning(f" Serial {serial_number} not found in SAP")
                else:
...
def post_invoice_to_sap_b1(invoice_data):
    ...
    try:
        ...
        if not sap.base_url or not sap.username or not sap.password:
            logging.warning(" SAP B1 configuration missing - cannot post invoice")
            return {
                'success': False,
